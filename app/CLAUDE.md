...
| Date | Change |
|---|---|
| 2026-06-05 | **Owner Nearby tab → local market map.** Owners now get `features/owner/OwnerNearbyScreen.tsx` on the Nearby tab instead of the player discover view — a map of their venues (lime pins) vs nearby competitor courts (grey pins), with a focus-venue selector, a radius control (5/10/25 mi), and four headline comparisons (price, rating, court count, competitor density) plus a per-competitor list with cheaper/pricier·higher/lower-rated·more/fewer-courts tags. Market math lives in `features/owner/utils/marketMetrics.ts` (`competitorsNear`/`marketSummary`/`compareToFocus`, pure + geo-based, reusing `shared/lib/geo` + `venueCoords`). No API change (uses `listOwnerVenues` + `listAllVenues`). New `owner.market.view` permission (synced to api/web, granted to `owner`); `App.tsx` branches the `nearby` case on it. |
