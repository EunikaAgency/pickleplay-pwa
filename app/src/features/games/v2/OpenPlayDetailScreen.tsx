...
import { skillBlockReason } from '../gameDisplay';
...
/* Open Play is a lobby like any other now (the merge): a `Game` of any gameType —
 * 'open' included — renders through GameDetailsScreen, so it gets the real roster,
 * group chat, and the server-side gender/skill/capacity guards on join. Before this,
 * 'open' had its own lobby-less screen whose roster was `interestedUsers` and whose
 * cap was a UI-only check on `targetPlayers`, so none of those guards ever ran.
 * `OpenPlaySession` (organizer-published, no booking) is still a separate thing and
 * keeps its own detail below. */
...
function AutoOpenPlayDetail({ id, chrome, onBack }: { id: string; chrome: V2ScreenChrome; onBack: () => void }) {
  // The id is either a Game or an OpenPlaySession — ask for the Game and let a
  // miss decide it's a session. Every gameType routes to the same lobby, so the
  // fetched game itself isn't needed here; GameDetailsScreen loads its own.
  const [kind, setKind] = useState<'loading' | 'game' | 'session'>('loading');
  // Reset during render, not inside the effect — a synchronous setState in an
  // effect body cascades renders. Same shape as OpenPlayChatScreen.
