// Relevance ranking for the Play tab's Discover feed.
//
...
//
// It is deliberately pure: no React, no fetch, no `Date.now()` (the caller passes
// `now`). Phase 2 lifts it onto the server unchanged, where it will additionally
// receive a friend set for the `social` signal.
//
...
import { haversineKm, formatDistance, type LatLng } from '../../shared/lib/geo';
...
export interface RankContext {
   ...
   *  dropped and its weight redistributed. Populated in Phase 2. */
...
function isInterestBased(g: ApiGame): boolean {
...
export function gameToPlayItem(g: ApiGame): PlayItem {
  const v = g.venue;
...
export function sessionToPlayItem(s: ApiOpenPlaySession): PlayItem {
  return {
    kind: 'session',
    id: s.id,
...
export function social(item: PlayItem, friendIds: Set<string>): number {
  ...
  const g = item.source as ApiGame;
