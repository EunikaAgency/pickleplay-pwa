...
const Peso = () => (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M7 21V4h5.5a4.5 4.5 0 0 1 0 9H7" /><line x1="4" y1="9" x2="14" y2="9" /><line x1="4" y1="13" x2="14" y2="13" /></svg>);
...
export function OwnerProfileScreen({ onNavigate, onLogout }: OwnerProfileScreenProps) {
  ...
  const manageRows: Row[] = [
    ...
    // Finance lives in the desktop Sidebar too, so the row is mobile/tablet-only
    // (`mtonly` — the scoped ≥1024px rule the Reservation row uses; Tailwind's
    // lg:hidden loses the specificity fight with .settings-item).
    ...(canReports ? [{ key: 'finance', icon: <Peso />, label: 'Finance & Receipts', sub: 'BIR receipts, VAT payable & tax breakdown', onClick: () => onNavigate('owner-finance'), className: 'mtonly' } as Row] : []),
