        ...
        <div className="flex flex-col">
          {posts.map((post) => (
            <FeedPostCard
              ...
              onShare={() => share(post)}
              onOpenActions={currentUser && post.author?.id === currentUser.id ? () => setActionTarget(post) : undefined}
            />
          ))}
        </div>
      )}
      ...
      <BottomSheet open={!!actionTarget} onClose={() => setActionTarget(null)} title="Post">
        {actionTarget && (
          <div className="flex flex-col gap-1 pb-2">
            <button
              type="button"
              onClick={() => { const t = actionTarget; setActionTarget(null); onNavigate('feed-post', { postId: t.id }); }}
            ...
            <button type="button" onClick={confirmDelete} className="w-full flex items-center gap-2.5 px-2 py-3 text-left text-[15px] font-semibold text-[var(--coral)] active:bg-[var(--surface-2)] rounded-xl">
