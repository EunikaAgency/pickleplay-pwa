...
export function CoachSubscribeScreen({ onNavigate, onBack, plan = 'coach' }: CoachSubscribeScreenProps) {
  ...
  const doSubscribe = async () => {
    ...
    setError(null);
    try {
      await subscribeToPartnerPlan(plan, { card, tierKey: selectedTierKey || undefined });
                ...
                <>
                  ...
                  >
                    ...
                    <span className="flex-1">
                      ...
                      <span className="block text-[12px] text-[var(--muted)]">Edit your public profile, rates, and per-venue pricing.</span>
                    </span>
                    <Icon name="chevron_right" size={20} />
                  </button>
                  <button
