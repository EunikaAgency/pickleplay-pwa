...
import { suggestPlaces, ApiError, type GeocodeSuggestion } from '../../lib/api';
 ...
 * only the suggestion list + dropdown UI. Shared by the venue create form, the
 * Location editor tab, and onboarding so they all behave identically.
 */
...
interface AddressAutocompleteProps {
  ...
  hint?: string;
  /**
   * Hold back suggestions while `value` is already a resolved place. Without it,
   * a caller that sets the text itself (e.g. from a GPS fix) trips the lookup and
   * pops a dropdown searching for the place the user just resolved.
   */
...
export function AddressAutocomplete({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      // The text already stands for a resolved place — nothing to search for.
  ...
  }, [value, country, suppressSuggestions]);
  ...
  return (
    <div className="field p-0!" ref={rootRef}>
      ...
      <div className="relative">
        ...
        {/* Overlaid, not in flow: an in-flow list resizes the form as it opens and
            closes, which in a centred layout shifts everything around it — enough
            that a click landing as the list closes misses whatever it aimed at. */}
        {showDropdown && (
          <ul
            id={listId}
            role="listbox"
            className="absolute left-0 right-0 top-full z-30 mt-1.5 max-h-[260px] overflow-auto rounded-xl border-[0.5px] border-[var(--hairline)] bg-[var(--surface)] shadow-lg"
          >
            {error ? (
