// The Discover relevance suite. Ported verbatim from app/src/features/games/
// playRanking.test.ts when the scorer moved server-side — same cases, same numbers.
// It is the safety net for that port: if the server ranks differently from the
// device did, one of these fails.
//
// Both `NOW` and `startAt` parse a bare 'YYYY-MM-DDTHH:MM' in the process timezone,
// so the suite is TZ-agnostic — it only asserts they agree with each other.
...
import {
  ...
  type FeedGame, type FeedSession, type PlayItem, type RankContext,
} from './playRanking.js';
...
function game(over: Partial<FeedGame> = {}): FeedGame {
  return { id: 'g1', gameType: 'doubles', date: '2026-07-10', startTime: '18:00', capacity: 4, participantCount: 0, ...over };
}
function session(over: Partial<FeedSession> = {}): FeedSession {
  return { id: 's1', title: 'Evening drills', date: '2026-07-10', startTime: '18:00', capacity: 8, joinedCount: 0, ...over };
}
...
describe('proximity', () => {
  ...
  it('rewards a nearby venue over a far one', () => {
    const near = gameToPlayItem(game({ venue: { lat: MAKATI_NEARBY[0], lng: MAKATI_NEARBY[1] } }));
    const far = gameToPlayItem(game({ venue: { lat: FAR[0], lng: FAR[1] } }));
...
describe('scoreItem — cold start', () => {
  ...
  it('reports distance only when both sides have coordinates', () => {
    ...
    const located = gameToPlayItem(game({ venue: { lat: MAKATI_NEARBY[0], lng: MAKATI_NEARBY[1] } }));
...
describe('why chips', () => {
  it('explains a close, on-level listing', () => {
    const item = gameToPlayItem(game({ skillMin: 3.0, skillMax: 3.5, venue: { lat: MAKATI_NEARBY[0], lng: MAKATI_NEARBY[1] } }));
  ...
  it('caps at two reasons so the chip row stays readable', () => {
    const item = gameToPlayItem(game({
      ...
      venue: { lat: MAKATI_NEARBY[0], lng: MAKATI_NEARBY[1] },
    }));
...
describe('adapters coerce the id', () => {
  // Found by running it, not by types. In-process, the open-play serializer hands
  // back `id` as a raw Mongo ObjectId — it only *looks* like a string after JSON
  // has been through it. The sort's `localeCompare` tiebreak threw on the real one
  // ("a.id.localeCompare is not a function") and took the whole feed down with it.
  const objectIdish = { toString: () => '6a4f11a748d823aa89f752d2' } as unknown as string;

  it('survives a non-string id from the session serializer', () => {
    const item = sessionToPlayItem(session({ id: objectIdish }));
    expect(item.id).toBe('6a4f11a748d823aa89f752d2');
    expect(() => sortScored([scoreItem(item, ctx())], 'best')).not.toThrow();
  });

  it('survives a non-string id from the game serializer', () => {
    expect(gameToPlayItem(game({ id: objectIdish })).id).toBe('6a4f11a748d823aa89f752d2');
  });
});
