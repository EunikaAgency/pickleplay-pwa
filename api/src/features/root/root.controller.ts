  ...
  const groups: Group[] = [
    ...
    },
    {
      ...
      endpoints: [
        ...
        { path: '/api/v1/open-play/:id/join', methods: ['POST'], description: 'Join an open-play session — waitlists when full', auth: 'user' },
        { path: '/api/v1/open-play/:id/leave', methods: ['POST'], description: 'Leave an open-play session', auth: 'user' },
    ...
    },
    {
      name: 'Games',
      endpoints: [
        { path: '/api/v1/games', methods: ['GET', 'POST'], description: 'List open-play games (filters: status, venueId, date; ?mine=true for created/joined). POST creates a fixed-venue game at a court the host has booked + paid — body takes venueId + bookingId (auth, player.games.create)' },
        { path: '/api/v1/games/:id', methods: ['GET', 'PATCH', 'DELETE'], description: 'Single game (GET). PATCH edits details (type/skill/name/capacity/visibility — venue + schedule are locked); DELETE removes it (both host-only, player.games.manage)' },
        { path: '/api/v1/games/:id/join', methods: ['POST'], description: 'Join a game — enforces capacity + one-per-player', auth: 'user' },
