  ...
  const groups: Group[] = [
    ...
    },
    {
      ...
      endpoints: [
        ...
        { path: '/api/v1/venues/:id/slot-overrides', methods: ['GET', 'POST'], description: 'Manual surge / slot price overrides. GET = active overrides (public, today onward; owner/staff see all with ?date, or a whole span with ?from&to — how the pricing grid loads a month in one call). POST = owner/staff set an absolute ₱/hr for a date+time window (optional courtId)' },
        ...
        { path: '/api/v1/venues/:id/bookings/:bookingId/attendance', methods: ['POST'], description: 'Record how a played-out booking ended — { attendance: attended | no_show }. A no-show keeps the booking confirmed (the venue keeps the payment, so it stays in revenue) and snapshots the venue noShowFee; re-markable to correct a mistake. 409 TOO_EARLY before the slot starts (owner or any active staff)', auth: 'user' },
    ...
    },
    {
      ...
      endpoints: [
        ...
        { path: '/api/v1/open-play', methods: ['POST'], description: 'Create a recurring open-play series + generate instances — body { title, venueId, daysOfWeek[], startTime, endTime?, capacity, price, weeksAhead }. Open to organizers (organizer.events.manage) AND to venue owners/staff for a venue they MANAGE — an owner running weekly play on their own courts needs that one capability, not the whole organizer role', auth: 'user' },
        { path: '/api/v1/open-play/mine', methods: ['GET'], description: "The caller's own open-play series + their session instances", auth: 'user' },
        { path: '/api/v1/open-play/series/:id', methods: ['PATCH'], description: 'Edit a SERIES — ?scope=future (default: the template + every occurrence from today on, so a price rise never rewrites what last week\'s players were charged) or ?scope=all (also past occurrences; for fixing a typo). Venue and recurrence days are not editable — that is a different series. Returns updatedSessions', auth: 'user' },
        { path: '/api/v1/open-play/:id', methods: ['PATCH'], description: 'Edit ONE occurrence ("just this week") — leaves the series template alone, and notifies everyone who already joined that session', auth: 'user' },
        { path: '/api/v1/open-play/series/:id/cancel', methods: ['PATCH'], description: 'Cancel a series + its future instances (organizer who created it, or whoever manages the venue)', auth: 'user' },
    ...
    },
    {
      name: 'Games',
      endpoints: [
        { path: '/api/v1/play/discover', methods: ['GET'], description: 'The Play tab\'s ranked Discover feed — merges public games + venue Open Play sessions into ONE relevance-ordered list (time fit 30% / proximity 25% / skill fit 20% / spots left 15% / friends 10%; a signal whose input is missing is dropped and the rest renormalised). ?section=open-play|events, ?lat=&lng= (the viewer\'s coords — skill + friends come from the token), ?pageSize=. Scores the whole upcoming catalogue and truncates after, so ranking is not limited to whatever was soonest. Listings the viewer already hosts/joined are excluded', sampleQuery: '?section=open-play' },
    ...
    },
    {
      ...
      endpoints: [
        ...
        { path: '/api/v1/bookings/:id/cancel', methods: ['POST'], description: 'Cancel a booking. Returns the applied refundQuote AND the settled `refund` outcome — { state: not_required | pending | completed, amount, feeDeducted }. Test mode reverses instantly; live mode parks the Payment in refund_pending for the refund queue below', auth: 'user' },
        { path: '/api/v1/bookings/:id/modify', methods: ['PATCH'], description: 'Reschedule a booking (date / time / court, max 3 times). The new slot is re-priced through the same ladder checkout uses, so the response carries the real `priceDelta` (positive = the player owes more, negative = a credit is due) plus the updated amount / serviceFeeAmount / balanceDue', auth: 'user' },
        ...
        { path: '/api/v1/payments/receipts', methods: ['POST'], description: 'Issue an official receipt by hand, for money taken outside a booking (counter sale, coaching paid in cash). Takes the same per-venue OR series and 12% VAT extraction as an auto-generated one — { venueId, payorName, category, description, amount, method, vatExempt?, discountCategory?, birInvoiceNumber? }. Requires owner.reports.view and ownership of the venue', auth: 'user' },
    ...
    },
    {
      ...
      endpoints: [
        ...
        { path: '/api/v1/notifications/:id', methods: ['PATCH', 'DELETE'], description: "PATCH flips one notification's read state — body { isRead } (defaults to true, so an empty body still means 'read'). DELETE removes it", auth: 'user' },
        { path: '/api/v1/notifications/:id/report', methods: ['POST'], description: 'Report a notification for moderation — body { reason? }; one row per (reporter, notification)', auth: 'user' },
    ...
    },
    {
      ...
      endpoints: [
        ...
        { path: '/api/v1/messages/conversations/:id/read', methods: ['POST'], description: 'Mark the thread read without reloading it (venue threads mark the whole venue side read)', auth: 'user' },
        { path: '/api/v1/messages/conversations/:id/unread', methods: ['POST'], description: 'Mark the thread unread again — rewinds the read mark to just before the last incoming message', auth: 'user' },
        { path: '/api/v1/messages/conversations/:id/report', methods: ['POST'], description: 'Report a thread for moderation — body { reason? }; one row per (reporter, thread)', auth: 'user' },
        { path: '/api/v1/messages/conversations/:id/typing', methods: ['POST'], description: 'Broadcast a typing indicator to the other side (no persistence)', auth: 'user' },
