...
function personView(u: any) {
  return u
    ? { id: String(u._id ?? u.id), displayName: u.displayName ?? 'Player', avatarUrl: u.avatarUrl ?? null }
    : null;
}
...
export async function listConversations(c: any) {
  ...
  const users = otherIds.length ? await User.find({ _id: { $in: otherIds } }).select('displayName avatarUrl').lean() : [];
...
export async function startConversation(c: any) {
  ...
  const other = await User.findById(userId).select('displayName avatarUrl').lean();
...
export async function getConversation(c: any) {
  ...
  const other = otherId ? await User.findById(otherId).select('displayName avatarUrl').lean() : null;
  ...
  return c.json({
    data: {
      ...
      messages: messages.map((m: any) => ({
        ...
        mine: String(m.senderId) === user.sub,
      })),
    },
  });
}
...
export async function sendMessage(c: any) {
  ...
  const now = new Date();
  const msg = await Message.create({ conversationId: conv._id, senderId: user.sub, body });
  ...
  if (recipientId) {
    ...
    publishUserEvent(recipientId, 'message.created', {
      conversationId: String(conv._id),
      message: { id: String(msg._id), senderId: String(user.sub), body, createdAt: now, mine: false },
    });
  ...
  return c.json({
    data: { id: String(msg._id), senderId: user.sub, body, createdAt: now, mine: true },
  }, 201);
}
