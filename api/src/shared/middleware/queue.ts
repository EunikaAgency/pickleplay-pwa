...
// answer "how many requests may the process work on at once?". Node runs one
// event loop against one Mongo pool, so an unbounded burst degrades everything
// together rather than failing fast.
//
// Measured on a 4-core box, 2000 simultaneous requests to /venues?limit=50:
//   no gate  -> all 2000 served in 12.3s, but /health peaked at 5.0s
//   this gate-> 579 served in 5.8s, 1421 shed, /health peaked at 1.45s
// Neither crashed. So this middleware does not buy survival at that scale — it
// buys BOUNDED LATENCY (fail fast instead of making everyone wait 12s+) and it
// bounds how much work Mongo and the event loop see at once. Raising maxQueued
// to 2048 was measurably worse on both counts: 771 still failed, now via the
// 10s timeout, and /health degraded to 14.8s. Don't "fix" shedding that way.
//
// The real fairness win is per-client: a flood from ONE identity is capped at
// maxConcurrentPerClient executing slots and its overflow is shed, while other
// callers keep their own slots and the shared queue. That is the case this
// middleware genuinely protects against.
//
// Excess requests WAIT rather than fail. Only a full queue (or a request that
// waited past the timeout) sheds a 503.
//
...
//
// The two per-client numbers are deliberately asymmetric, and the reason is the
// anonymous case. An anonymous caller is identified by IP, so a whole office,
// campus, or mobile carrier's CGNAT collapses into ONE client key. Capping how
// many of their requests may *execute* at once (16) is fine — it only bounds
// their share of the event loop and the Mongo pool. Capping how many may *wait*
// must be generous (256, half the shared queue), because a waiter costs nothing
// but a promise, and shedding one means a real person sees a failed request. A
// tight per-client queue reproduces the exact bug this middleware exists to
// prevent: a burst from a shared IP failing for everybody behind it. Measured:
// with a 64-deep per-client queue, a 200-request burst from one IP shed 83 of
// them; at 256 it sheds none, while a genuine 2000-request flood still gets cut
// off and still leaves half the shared queue free for everyone else.
export const DEFAULT_QUEUE_CONFIG: QueueConfig = {
  maxConcurrent: num(process.env.API_MAX_CONCURRENT, 64),
  maxQueued: num(process.env.API_MAX_QUEUED, 512),
  maxConcurrentPerClient: num(process.env.API_MAX_CONCURRENT_PER_CLIENT, 16),
  maxQueuedPerClient: num(process.env.API_MAX_QUEUED_PER_CLIENT, 256),
 ...
 *  - /health — monitoring has to answer precisely when we are saturated. Note
 *    this exempts /health from the QUEUE, not from the event loop: under a heavy
 *    backlog it still slows down (1.45s at 2000 concurrent, measured). It just
 *    never waits behind the queue for a slot.
