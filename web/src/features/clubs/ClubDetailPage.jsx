...
import { getClub, getUser } from '../../shared/data/index.js';
...
export default function ClubDetailPage() {
  ...
  const admins = club.admins.flatMap(id => { const u = getUser(id); return u ? [u] : []; });
  const members = club.memberIds.flatMap(id => { const u = getUser(id); return u ? [u] : []; });
      ...
      <section className="bg-gradient-to-r from-[#2E5BFF] to-[#0040E0] px-5 py-12 text-white">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-4 flex items-center gap-4">
            <img src={club.avatarUrl} alt="" className="size-20 rounded-2xl border-4 border-white/30 object-cover shadow-lg" />
            <div>
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          <div className="lg:col-span-2 space-y-6">
              ...
              <div className="rounded-2xl bg-white p-6 shadow-lg">
                ...
                <div className="mt-4 flex flex-wrap gap-3">{admins.map(a => (
                  <div key={a.id} className="flex items-center gap-3 rounded-2xl border-2 border-surface-variant p-3">
                    <img src={a.avatar} alt="" className="size-10 rounded-full object-cover" />
            ...
            <div className="rounded-2xl bg-white p-6 shadow-lg">
              ...
              <div className="mt-4 flex flex-wrap gap-2">{members.slice(0,12).map(m => (
                <img key={m.id} src={m.avatar} alt={m.firstName} title={`${m.firstName} ${m.lastName}`} className="size-10 rounded-full border-2 border-white object-cover shadow-sm" />
              ))}{members.length > 12 && <div className="flex size-10 items-center justify-center rounded-full bg-surface-container-high text-base font-extrabold">+{members.length-12}</div>}</div>
            </div>
          </div>
          <aside className="space-y-4">
            <div className="rounded-2xl bg-white p-6 shadow-lg text-center">
              ...
              <button type="button" className="mt-4 h-14 w-full rounded-2xl bg-[#C1F100] text-base font-extrabold text-[#374D00] shadow-lg hover:scale-105 active:scale-95 transition-transform">Join Club 🤝</button>
            </div>
            <div className="rounded-2xl bg-white p-6 shadow-lg">
